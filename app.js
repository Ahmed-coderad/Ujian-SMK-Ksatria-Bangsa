...
/* Daftar siswa terdaftar (roster), jadwal ujian, dan daftar SEMUA mapel
   yang tersedia (bawaan + mapel tambahan buatan admin/impor dokumen),
   dimuat dari penyimpanan terpusat dan (jika tersedia) diperbarui secara
   real-time sehingga perubahan admin langsung terlihat di layar login
   tanpa perlu memuat ulang halaman. */
let roster = [];
let schedule = {};
let classSubjectMap = Object.assign({}, CLASS_SUBJECT_MAP);
let allExams = Object.assign({}, EXAMS);
function rebuildClassSubjectMap(customExams) {
  const map = {};
  Object.entries(CLASS_SUBJECT_MAP).forEach(([k, list]) => { map[k] = [...list]; });
  Object.values(customExams || {}).forEach(ex => {
...
function subscribeLoginData() {
  if (Cloud.ready()) {
    Cloud.watchRoster(list => { roster = list; if (state.screen === "login") render(); });
    Cloud.watchSchedule(sc => { schedule = sc; if (state.screen === "login") render(); });
    Cloud.watchCustomExams(custom => {
      allExams = Object.assign({}, EXAMS);
      Object.entries(custom || {}).forEach(([k, d]) => { allExams[k] = Object.assign({ id: k }, d); });
      classSubjectMap = rebuildClassSubjectMap(custom);
      if (state.screen === "login") render();
    });
  } else {
    getRosterShared().then(list => { roster = list; if (state.screen === "login") render(); });
    getScheduleShared().then(sc => { schedule = sc; if (state.screen === "login") render(); });
    getCustomExamsShared().then(custom => {
      allExams = Object.assign({}, EXAMS);
      Object.entries(custom || {}).forEach(([k, d]) => { allExams[k] = Object.assign({ id: k }, d); });
      classSubjectMap = rebuildClassSubjectMap(custom);
      if (state.screen === "login") render();
...
/* Contoh nama pada placeholder kolom "Nama Lengkap" di formulir login,
   ditampilkan bergantian dengan efek animasi mengetik agar terasa hidup
   dan tidak disangka nama sungguhan yang harus diisi persis. */
const LOGIN_NAME_EXAMPLES = ["SOPIAN", "NAZWA AZILA GUNAWAN", "ZAHIRA SAKINA"];

/* Efek animasi "mengetik" pada atribut placeholder sebuah <input>: setiap
   kata di `words` diketik huruf demi huruf, berhenti sejenak, dihapus
   huruf demi huruf, lalu lanjut ke kata berikutnya — berulang terus.
   Mengembalikan fungsi untuk menghentikan animasi (dipanggil saat form
   login dibangun ulang, agar tidak ada animasi "hantu" menumpuk). */
function startPlaceholderTypewriter(input, prefix, words, opts = {}) {
  ...
  let wordIdx = 0, charIdx = 0, deleting = false, timerId = null, stopped = false;
  function tick() {
    if (stopped) return;
    const word = words[wordIdx];
    if (!deleting) {
      charIdx++;
      input.placeholder = prefix + word.slice(0, charIdx);
      if (charIdx >= word.length) { deleting = true; timerId = setTimeout(tick, pauseAfterType); return; }
      timerId = setTimeout(tick, typeSpeed);
    } else {
      charIdx--;
      input.placeholder = prefix + word.slice(0, Math.max(charIdx, 0));
      if (charIdx <= 0) {
        deleting = false;
        wordIdx = (wordIdx + 1) % words.length;
        timerId = setTimeout(tick, pauseAfterDelete);
        return;
      }
...
function renderLogin() {
  ...
  if (stopLoginTypewriter) { stopLoginTypewriter(); stopLoginTypewriter = null; }
  const nameField = el("input", { type: "text", placeholder: "Contoh: ", autocomplete: "off" });
  stopLoginTypewriter = startPlaceholderTypewriter(nameField, "Contoh: ", LOGIN_NAME_EXAMPLES);
  const kelasField = el("select", {}, [
    el("option", { value: "" }, "-- Pilih Kelas --"),
    ...Object.keys(classSubjectMap).map(k => el("option", { value: k }, k))
  ]);
  ...
  const scheduleBox = el("div", { class: "hidden" });

  function updateScheduleBox() {
    const kelas = kelasField.value, mapel = mapelField.value;
    if (!kelas || !mapel) { scheduleBox.className = "hidden"; return; }
    const examId = `${kelas}__${mapel}`;
    const sc = schedule[scheduleKey(examId)];
    const durMin = effectiveDurationMinutes(sc, allExams[examId]);
    if (!sc || !sc.startAt) {
      // Jadwal WAJIB diisi admin — tanpa jadwal, peserta tidak dapat login.
      scheduleBox.className = "alert alert-danger";
      scheduleBox.textContent = "\u26D4 Jadwal mulai ujian untuk kelas & mata pelajaran ini belum ditentukan oleh admin. Anda belum dapat login untuk mengerjakan ujian ini — hubungi admin agar jadwal diatur terlebih dahulu di menu Jadwal Ujian.";
      return;
    }
    const startAt = new Date(sc.startAt);
    const fmt = startAt.toLocaleString("id-ID", { dateStyle: "full", timeStyle: "short" });
    if (new Date() < startAt) {
      scheduleBox.className = "alert alert-info";
      scheduleBox.textContent = `\u23F3 Ujian ini dijadwalkan mulai: ${fmt}. Durasi pengerjaan: ${durMin} menit. Anda belum dapat masuk sebelum waktu tersebut.`;
    } else {
      scheduleBox.className = "alert alert-success";
      scheduleBox.textContent = `\u2713 Ujian telah dibuka sejak ${fmt}. Durasi pengerjaan: ${durMin} menit.`;
    }
  }
  ...
  kelasField.addEventListener("change", () => {
    ...
    (classSubjectMap[kelas] || []).forEach(m => mapelField.appendChild(el("option", { value: m }, m)));
    updateScheduleBox();
  });
  mapelField.addEventListener("change", updateScheduleBox);
  ...
  startBtn.addEventListener("click", async () => {
    ...
    const examId = `${kelas}__${mapel}`;
    const exam = examId in allExams ? await getEffectiveExam(examId) : null;
    ...
    // Nama wajib terdaftar lebih dulu oleh admin (Data Siswa & Kelas).
    // PENTING: satu nama bisa saja terdaftar lebih dari sekali (mis. siswa
    // yang sama mengikuti ujian untuk kelas yang sama tetapi mata pelajaran
    // berbeda) sebagai baris roster TERPISAH. Karena itu pencarian harus
    // mencocokkan nama + kelas + mapel SEKALIGUS, bukan berhenti pada
    // kecocokan nama pertama yang ditemukan — jika tidak, siswa dengan nama
    // yang sama akan selalu tersangkut ke baris pertamanya saja (mis. mata
    // pelajaran lain) dan gagal login untuk mata pelajaran yang sebenarnya
    // sudah benar terdaftar.
    if (nama.length >= 3) {
      if (!roster.length) {
        errors.push("Belum ada data siswa terdaftar dari admin. Hubungi admin untuk mendaftarkan nama Anda terlebih dahulu.");
      } else {
        const candidates = roster.filter(r => normalizeName(r.nama) === normalizeName(nama));
        if (!candidates.length) {
          errors.push(`Nama "${nama}" belum terdaftar di data siswa. Hubungi admin untuk didaftarkan terlebih dahulu (menu Data Siswa & Kelas).`);
        } else {
          const exact = candidates.find(r => (!kelas || r.kelas === kelas) && (!mapel || r.mapel === mapel));
          if (!exact) {
            const sameKelas = kelas ? candidates.filter(r => r.kelas === kelas) : candidates;
            if (kelas && !sameKelas.length) {
              const kelasTerdaftar = [...new Set(candidates.map(c => c.kelas))].join(", ");
              errors.push(`Nama "${nama}" terdaftar di kelas ${kelasTerdaftar}, bukan ${kelas}. Periksa kembali pilihan kelas Anda.`);
            } else if (mapel) {
              const mapelTerdaftar = [...new Set(sameKelas.map(c => c.mapel))].join(", ");
              errors.push(`Nama "${nama}" di kelas ${kelas} terdaftar untuk mata pelajaran: ${mapelTerdaftar} — bukan ${mapel}. Jika Anda memang mengikuti ujian ${mapel}, minta admin menambahkan baris pendaftaran terpisah untuk mata pelajaran ini di menu Data Siswa & Kelas.`);
            }
          }
        }
      }
    ...
    // Ujian hanya dapat diakses pada jadwal yang ditentukan admin.
    // Jadwal WAJIB dikonfigurasi lebih dulu oleh admin (menu Jadwal
    // Ujian); selama belum diisi, peserta tidak dapat login sama sekali.
    if (kelas && mapel) {
      const sc = schedule[scheduleKey(examId)];
      if (!sc || !sc.startAt) {
        errors.push("Jadwal mulai ujian untuk kelas & mata pelajaran ini belum ditentukan oleh admin. Hubungi admin agar jadwal diatur terlebih dahulu di menu Jadwal Ujian sebelum Anda dapat login.");
      } else if (new Date() < new Date(sc.startAt)) {
        const fmt = new Date(sc.startAt).toLocaleString("id-ID", { dateStyle: "full", timeStyle: "short" });
        errors.push(`Ujian belum dibuka. Jadwal mulai: ${fmt}.`);
      }
    }
    ...
    state.durationMinutesUsed = effectiveDurationMinutes(schedule[scheduleKey(examId)], exam);
    state.timeLeftSec = state.durationMinutesUsed * 60;
    if (stopLoginTypewriter) { stopLoginTypewriter(); stopLoginTypewriter = null; }
...
function attachAntiCheat() {
  ...
  document.addEventListener("copy", onCopyPasteAttempt);
  document.addEventListener("paste", onCopyPasteAttempt);
  document.addEventListener("cut", onCopyPasteAttempt);
}
function detachAntiCheat() {
  ...
  document.removeEventListener("copy", onCopyPasteAttempt);
  document.removeEventListener("paste", onCopyPasteAttempt);
  document.removeEventListener("cut", onCopyPasteAttempt);
}
function preventDefaultIfExam(e){ if (state.screen === "exam") e.preventDefault(); }
function onCopyPasteAttempt(e) {
  if (state.screen !== "exam") return;
  e.preventDefault();
  const label = e.type === "paste" ? "menempel (paste) teks" : e.type === "cut" ? "memotong (cut) teks" : "menyalin (copy) teks";
  registerStrike(`terdeteksi ${label} pada halaman ujian`);
}
...
function registerStrike(reason) {
  ...
  if (state.exitStrikes >= MAX_EXIT_STRIKES) {
    showModal({
      ...
      body: `Sistem mendeteksi ${state.exitStrikes} kali pelanggaran tata tertib ujian (batas maksimal ${MAX_EXIT_STRIKES} kali dilampaui — pelanggaran terakhir: ${reason}). Sesuai tata tertib ujian, jawaban Anda akan dikirim secara otomatis sekarang.`,
    ...
    showModal({
      ...
      title: "\u26A0 ALARM — Pelanggaran Tata Tertib Ujian Terdeteksi",
      body: `Sistem mendeteksi ${reason}. Ini adalah pelanggaran ke-${state.exitStrikes} dari maksimal ${MAX_EXIT_STRIKES} kali. Jika batas terlampaui, seluruh jawaban akan otomatis dikirim dan ujian dianggap selesai. Tetaplah pada halaman ujian, jangan berpindah tab, dan jangan menyalin/menempel teks.`,
...
/* ---- Alarm kecurangan: bunyi peringatan + kilatan layar merah ----
   Dipicu setiap kali registerStrike() mendeteksi pelanggaran (pindah
   tab, jendela kehilangan fokus, keluar layar penuh, atau mencoba
   menyalin/menempel teks) agar peserta langsung sadar bahwa tindakan
   tersebut sudah tercatat sistem. */
function triggerCheatAlarm() {
  ...
  } catch (e) { /* audio tidak tersedia/diblokir browser — abaikan, alarm visual tetap tampil */ }
...
function startProctorMedia() {
  ...
  if (video && state.mediaStream) {
    ...
    video.muted = true; video.volume = 0; // peserta tidak boleh mendengar rekaman mikrofonnya sendiri
    video.play().catch(()=>{});
  }
...
function captureSnapshot() {
  ...
  // Cuplikan foto SENGAJA hanya disimpan lokal (IndexedDB) di perangkat
  // ini, sama seperti rekaman audio — tidak pernah dikirim ke penyimpanan
  // terpusat (lihat catatan privasi di firebase-config.js & cloud.js).
  ExamDB.saveSnapshot(snap).catch(()=>{});
}
...
function renderExam() {
  ...
  if (state.activeSection === "pg") {
    ...
    qcard.appendChild(el("div", { class: "qtext" }, item.q));
    qcard.appendChild(el("div", { class: "essay-label" }, "\u270E Jawaban Anda"));
    qcard.appendChild(el("div", { class: "essay-hint" }, "Tulis jawaban dengan kalimat lengkap dan jelas. Jawaban tersimpan otomatis saat Anda mengetik."));
    const ta = el("textarea", { class: "essay-input", placeholder: "Tulis jawaban Anda di sini...", rows: "7" }, state.essayAnswers[i] || "");
    const wc = el("div", { class: `word-count ${((state.essayAnswers[i]||"").trim().split(/\s+/).filter(Boolean).length) > 0 && (state.essayAnswers[i]||"").trim().split(/\s+/).filter(Boolean).length < 5 ? "low" : ""}` }, `${(state.essayAnswers[i]||"").trim() ? state.essayAnswers[i].trim().split(/\s+/).length : 0} kata`);
    ta.addEventListener("input", () => {
      state.essayAnswers[i] = ta.value;
      const n = ta.value.trim() ? ta.value.trim().split(/\s+/).length : 0;
      wc.textContent = `${n} kata`;
      wc.classList.toggle("low", n > 0 && n < 5);
  ...
  // Transparansi ke peserta bahwa mikrofon aktif untuk pengawasan, TANPA
  // memberi mereka akses dengar/putar ulang — rekaman hanya bisa diputar
  // dari dasbor admin (lihat admin.js) setelah ujian selesai.
  camWrap.appendChild(el("div", { class: "mic-badge" }, "\uD83C\uDF99\uFE0F Mikrofon aktif untuk pengawasan \u2014 rekaman hanya dapat diputar oleh admin"));
  wrap.appendChild(camWrap);

  setTimeout(() => {
    const v = document.getElementById("proctorVideo");
    if (v && state.mediaStream) {
      v.srcObject = state.mediaStream;
      // Jaga-jaga ganda: pastikan tidak ada audio yang terdengar oleh
      // peserta sendiri, sekalipun atribut HTML "muted" gagal diterapkan.
...
async function finishExam(reason) {
  ...
  const mc = gradeMultipleChoice(state.exam, state.mcAnswers);
  const essay = gradeEssay(state.exam, state.essayAnswers);
  const finalScore = computeFinalScore(mc.score, essay.score);
  ...
  const attempt = {
    ...
    timeUsedSec: (state.durationMinutesUsed*60) - Math.max(state.timeLeftSec,0),
    ...
    essayRawScore: essay.rawScore,
    essayRawMax: essay.rawMax,
  ...
  try { await ExamDB.saveAttempt(attempt); } catch (e) { console.error("Gagal menyimpan cadangan lokal:", e); }
  try { if (Cloud.ready()) await Cloud.saveAttempt(attempt); } catch (e) { console.error("Gagal mengirim hasil ke server pusat:", e); }
...
function renderDone() {
  ...
  const rows = [
    ...
    ["Skor Essay (estimasi otomatis)", `${a.essayRawScore}/${a.essayRawMax} (skor ${a.essayScore})`],
    ["Nilai Akhir (estimasi)", `${a.finalScore} / 100`],
  ];
