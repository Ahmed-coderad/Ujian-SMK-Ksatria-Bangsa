...
/* Daftar siswa terdaftar (roster) & jadwal ujian, dimuat dari
   penyimpanan terpusat dan (jika tersedia) diperbarui secara
   real-time sehingga perubahan admin langsung terlihat di layar
   login tanpa perlu memuat ulang halaman. */
let roster = [];
let schedule = {};
...
function renderLogin() {
  ...
  function updateScheduleBox() {
    ...
    if (new Date() < startAt) {
      ...
      scheduleBox.textContent = `\u23F3 Ujian ini dijadwalkan mulai: ${fmt}. Anda belum dapat masuk sebelum waktu tersebut.`;
    } else {
      scheduleBox.className = "alert alert-success";
      scheduleBox.textContent = `\u2713 Ujian telah dibuka sejak ${fmt}.`;
    }
  }
