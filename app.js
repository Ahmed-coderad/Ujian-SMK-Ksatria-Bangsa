  ...
  startBtn.addEventListener("click", async () => {
    ...
    if (nama.length >= 3) {
      if (!roster.length) {
        ...
        const match = roster.find(r => normalizeName(r.nama) === normalizeName(nama));
        if (!match) {
          ...
          if (kelas && match.kelas && match.kelas !== kelas) errors.push(`Nama "${nama}" terdaftar di kelas ${match.kelas}, bukan ${kelas}. Periksa kembali pilihan kelas Anda.`);
          if (mapel && match.mapel && match.mapel !== mapel) errors.push(`Nama "${nama}" terdaftar untuk mata pelajaran ${match.mapel}, bukan ${mapel}.`);
        }
      }
    }
