...
function renderLogin() {
  ...
  function updateScheduleBox() {
    ...
    if (!sc || !sc.startAt) { scheduleBox.className = "hidden"; return; }
  ...
  startBtn.addEventListener("click", async () => {
    ...
    if (kelas && mapel) {
      const sc = schedule[scheduleKey(examId)];
      if (sc && sc.startAt && new Date() < new Date(sc.startAt)) {
...
function captureSnapshot() {
  ...
  if (Cloud.ready()) Cloud.saveSnapshot(state.attemptId, snap).catch(()=>{});
}
