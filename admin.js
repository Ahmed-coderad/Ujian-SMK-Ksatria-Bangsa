...
async function renderBank() {
  ...
  exam.essay.forEach((q, i) => {
    const box = el("div", { class: "essay-review" });
    box.appendChild(el("div", { style:"font-weight:700;margin-bottom:8px;display:flex;align-items:center;" }, [el("span", { class: "qnum" }, String(i+1)), `Soal No. ${i+1}`]));
...
async function renderSchedule() {
  ...
  wrap.appendChild(el("p", { class: "muted" }, "Tentukan tanggal, bulan, tahun, dan jam pembukaan tiap paket ujian. WAJIB diisi: selama jadwal belum ditentukan di sini, peserta TIDAK DAPAT login untuk paket ujian tersebut sama sekali. Setelah waktu yang ditentukan tiba, peserta dapat login dan mengerjakan ujian."));
  if (!Cloud.ready()) {
  ...
  const table = el("table", { class: "ledger" });
  table.appendChild(el("tr", {}, ["Kelas", "Mata Pelajaran", "Waktu Mulai Ujian", "Status", ""].map(h => el("th", {}, h))));
...
function exportToExcel(rows) {
  ...
  const perExamRows = allClassSubjectKeys()
    .map(key => ({ key, exam: EXAMS[key], list: sorted.filter(a => a.examId === key) }))
    .filter(g => g.list.length);

  const ringkasanAOA = [
    [`${SCHOOL_NAME.toUpperCase()} — REKAP HASIL UJIAN`],
    [SCHOOL_PROGRAM],
    [filterLabel],
    [`Digenerate otomatis oleh Portal Ujian pada ${generatedAt}`],
    [],
    ["STATISTIK UMUM"],
    ["Total Peserta", totalPeserta],
    ["Rata-rata Nilai Akhir", rataAkhir],
    ["Nilai Akhir Tertinggi", tertinggi],
    ["Nilai Akhir Terendah", terendah],
    ["Ujian Dihentikan Otomatis (Pelanggaran)", dihentikan],
    ["Total Insiden Pelanggaran Tercatat", totalInsiden],
    [],
    ["REKAP PER KELAS & MATA PELAJARAN"],
    ["Kelas", "Mata Pelajaran", "Guru", "Jumlah Peserta", "Rata-rata Skor PG", "Rata-rata Skor Essay", "Rata-rata Nilai Akhir", "Pelanggaran"]
  ];
  const perExamHeaderRow = ringkasanAOA.length; // 0-indexed row number of the header just pushed
  perExamRows.forEach(g => {
    ringkasanAOA.push([
      g.exam.kelas, g.exam.mapel, g.exam.guru, g.list.length,
      avgOf(g.list, "mcScore"), avgOf(g.list, "essayScore"), avgOf(g.list, "finalScore"),
      g.list.filter(a => a.reason === "pelanggaran").length
    ]);
  });

  const wsSummary = XLSX.utils.aoa_to_sheet(ringkasanAOA);
  wsSummary["!cols"] = [{ wch: 30 }, { wch: 22 }, { wch: 26 }, { wch: 14 }, { wch: 16 }, { wch: 18 }, { wch: 18 }, { wch: 12 }];
  wsSummary["!merges"] = [
    { s: { r: 0, c: 0 }, e: { r: 0, c: 7 } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: 7 } },
    { s: { r: 2, c: 0 }, e: { r: 2, c: 7 } },
  ...
  if (perExamRows.length) {
    wsSummary["!autofilter"] = { ref: XLSX.utils.encode_range({ s: { r: perExamHeaderRow, c: 0 }, e: { r: perExamHeaderRow + perExamRows.length, c: 7 } }) };
  }
  XLSX.utils.book_append_sheet(wb, wsSummary, "Ringkasan");

  /* ---------- SHEET 2: REKAP NILAI (detail, terurut kelas > mapel > nama) ---------- */
  const detailHeader = ["No", "Nama Siswa", "Kelas", "Mata Pelajaran", "Guru", "Skor PG", "Benar/Total PG", "Skor Essay (Estimasi)", "Nilai Akhir", "Jumlah Pelanggaran", "Status Akhir", "Durasi Dipakai", "Waktu Kumpul", "Nomor Dokumen"];
  const detailAOA = [
    [`${SCHOOL_NAME} — Rekap Nilai Detail (${filterLabel})`],
    [],
    detailHeader
  ];
  sorted.forEach((a, i) => {
    detailAOA.push([
      i + 1, a.nama, a.kelas, a.mapel, a.guru,
      a.mcScore, `${a.mcCorrect}/${a.mcTotal}`, a.essayScore, a.finalScore,
      a.exitStrikes, a.status, fmtSec(a.timeUsedSec), fmtDateExcel(a.submittedAt), a.docNumber
    ]);
  });
  const wsDetail = XLSX.utils.aoa_to_sheet(detailAOA);
  wsDetail["!cols"] = [
    { wch: 4 }, { wch: 26 }, { wch: 10 }, { wch: 22 }, { wch: 24 },
    { wch: 9 }, { wch: 14 }, { wch: 16 }, { wch: 11 }, { wch: 12 },
    { wch: 22 }, { wch: 13 }, { wch: 18 }, { wch: 16 }
  ];
  wsDetail["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: detailHeader.length - 1 } }];
  const headerRowIdx = 2; // baris ke-3 (0-indexed) berisi header tabel
  wsDetail["!autofilter"] = { ref: XLSX.utils.encode_range({ s: { r: headerRowIdx, c: 0 }, e: { r: headerRowIdx + sorted.length, c: detailHeader.length - 1 } }) };
  // Format angka rapi (tanpa desimal) untuk kolom skor: Skor PG(5), Skor Essay(7), Nilai Akhir(8), Jml Pelanggaran(9)
  [5, 7, 8, 9].forEach(col => {
    for (let r = headerRowIdx + 1; r <= headerRowIdx + sorted.length; r++) {
      const cellRef = XLSX.utils.encode_cell({ r, c: col });
      if (wsDetail[cellRef]) wsDetail[cellRef].z = "0";
    }
  });
  XLSX.utils.book_append_sheet(wb, wsDetail, "Rekap Nilai");
