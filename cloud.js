...
const Cloud = {
  ...
  /* ---------------- MATA PELAJARAN TAMBAHAN (dibuat admin manual atau via impor dokumen) ----------------
     Berbeda dari examOverrides (yang menimpa sebagian data paket ujian BAWAAN di data.js),
     customExams menyimpan paket ujian LENGKAP yang sama sekali baru — sehingga admin bisa
     menambah mata pelajaran/kelas baru tanpa mengubah kode sumber sama sekali. */
  async getCustomExams() {
    if (!this.ready()) return {};
    return (await restGet("customExams")) || {};
  },
  watchCustomExams(cb) { return watchPath("customExams", val => cb(val || {})); },
  async saveCustomExam(examId, data) {
    if (!this.ready()) throw new Error("Penyimpanan terpusat belum aktif.");
    await restPut(`customExams/${cssKey(examId)}`, data);
  },
  async deleteCustomExam(examId) {
    if (!this.ready()) throw new Error("Penyimpanan terpusat belum aktif.");
    await restDelete(`customExams/${cssKey(examId)}`);
  },

  /* ---------------- JADWAL MULAI UJIAN (wajib diisi admin) + DURASI PER MAPEL ---------------- */
  async getSchedule() {
    if (!this.ready()) return {};
  ...
  async setScheduleDuration(examId, minutesOrNull) {
    if (!this.ready()) throw new Error("Penyimpanan terpusat belum aktif.");
    await restPut(`examSchedule/${cssKey(examId)}/durationMinutes`, minutesOrNull || null);
  },
