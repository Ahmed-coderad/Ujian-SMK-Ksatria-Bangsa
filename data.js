...
/* Util: hitung skor pilihan ganda.
   PENTING: menerima objek soal EFEKTIF (`exam`) langsung — bukan
   mencari ulang dari EXAMS bawaan berdasarkan id — supaya perubahan
   admin (override) maupun mata pelajaran tambahan (impor dokumen/
   dibuat manual, yang sama sekali tidak ada di EXAMS) tetap dinilai
   dengan benar sesuai soal yang sesungguhnya dikerjakan siswa. */
function gradeMultipleChoice(exam, answers) {
  ...
  const score = exam.mc.length ? Math.round((correct / exam.mc.length) * 100) : 0;
...
/* Util: heuristic auto-grading utk essay (skor awal, tetap perlu ditinjau guru).
   Sama seperti gradeMultipleChoice: menerima objek soal EFEKTIF langsung.
   PENTING: jumlah soal essay & maxScore tiap soal kini bisa berbeda-beda
   (soal bisa ditambah/dihapus admin, atau diimpor dari dokumen dengan
   jumlah bebas), jadi skor akhir SELALU dinormalisasi ke skala 0-100
   berdasarkan total maxScore sesungguhnya — bukan diasumsikan selalu 100. */
function gradeEssay(exam, answers) {
  let rawScore = 0;
  let rawMax = 0;
  const detail = [];
  exam.essay.forEach((item, i) => {
    ...
    const score = Math.round(raw * maxScore);
    rawScore += score;
    rawMax += maxScore;
    detail.push({ index: i, wordCount, kwHits, kwTotal: item.keywords.length, score, maxScore });
  });
  const score = rawMax ? Math.round((rawScore / rawMax) * 100) : 0; // dinormalisasi 0-100, sejajar dengan skor PG
  return { score, rawScore, rawMax, detail };
}
...
/* Membagi rata total 100 poin essay ke seluruh soal essay suatu paket
   (dipakai admin.js setelah menambah/menghapus soal essay, agar total
   maxScore tetap 100 tanpa admin perlu menghitung manual). */
function redistributeEssayScores(essayArr) {
  const n = essayArr.length;
  if (!n) return;
  const base = Math.floor(100 / n);
  const rem = 100 - base * n;
  essayArr.forEach((q, i) => { q.maxScore = base + (i < rem ? 1 : 0); });
}

if (typeof module !== "undefined") {
  module.exports = { SCHOOL_NAME, SCHOOL_PROGRAM, DEFAULT_TEACHER, EXAM_DURATION_MINUTES, MAX_EXIT_STRIKES, MC_WEIGHT, ESSAY_WEIGHT, CLASS_SUBJECT_MAP, EXAMS, gradeMultipleChoice, gradeEssay, computeFinalScore, redistributeEssayScores };
}
