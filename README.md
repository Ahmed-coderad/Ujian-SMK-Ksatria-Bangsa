...
- **Dasbor admin real-time**: rekap nilai per kelas/mapel, detail tiap peserta
  ...
  admin terpusat.
- **Ekspor Excel (.xlsx) yang rapi & terorganisir**: dua sheet dalam satu
  berkas — **Ringkasan** (statistik umum & rekap rata-rata per kelas/mapel
  dengan judul gabungan sel) dan **Rekap Nilai** (tabel detail seluruh
  peserta terurut per kelas > mapel > nama, lengkap dengan filter otomatis,
  lebar kolom yang disesuaikan, dan format angka rapi pada kolom skor).
